...
DICT_PATH = u'/Users/abiu/Downloads/LDOCE6/L6mp3.mdx'
...
class Ldoce6(MdxService):
    ...
    # @export([u'去头', u' withoutHead'])
    # def fld_without_head(self):
    #     html = self.get_default_html()
    #     p = re.compile(r'\[sound:(_.*?)]<img.*?>')
    #     html = p.sub(
    #         u'<div data-rel="\\1" role="dict_audio_js"><img src="_uk_pron.png.png"></div>', html)
    #     return self._css(html)
    ...
    def _fld_voice(self, html, voice):
        ...
        return ''
    # 如果用到了释义 ,即可以用这个函数,释义里已有音频
    def _fld_voice_local(self, html, voice):
        """获取发音字段"""
        for regexp in LANG_TO_REGEXPS[voice]:
            match = regexp.search(html)
            if match:
                name = match.group(1)
                name = name.replace(name, u'_' + name.split('/')[-1])
                return self.get_anki_label(name, 'audio')
        return ''

    @export('BRE_PRON')
    def fld_voicebre(self):
        return self._fld_voice_local(self.get_html(), 'br')

    @export('AME_PRON')
    def fld_voiceame(self):
        return self._fld_voice_local(self.get_html(), 'us')
    ...
    def fld_image(self):
        html = self.get_html()
        m = re.search(
            r'<span class="imgholder"><img src="(.*?)".*?></span>', html)
        if m:
    ...
    def fld_definate(self):
        m = m = re.findall(
            r'<span class="def"\s*.*>\s*.*<\/span>', self.get_html())
        if m:
            soup = parse_html(m[0])
